...
#                        use_listings_cache=False,
        ...
        outpath = path.replace("akash-thumper-v1-training-images", f"/home/ray/training_data/images/")
        s3.get(path, path.replace("akash-thumper-v1-training-images", f"/home/ray/training_data/images/"))
...
@ray.remote( num_cpus=3, num_gpus=1 , memory= 23.5* 1000 * 1024 * 1024) #10gb memory reserved 
class Actor:
    def __init__(self, args, files):
        from llava.model.builder import load_pretrained_model
        from llava.utils import disable_torch_init
        from llava.mm_utils import process_simages, tokenizer_image_token, get_model_name_from_path
...
#                        use_listings_cache=False,
        ...
        dataloader = DataLoader(dataset, batch_size=self.args.batch_size, shuffle=False, num_workers=self.args.dlworker)
        with torch.inference_mode():
            for i_batch, sample_batched in enumerate(dataloader):
                try:
                    ...
                    if i_batch % 100 ==0:
                        ...
                        filepath= f"/home/ray/train_data/tempcaption_{i_batch}_{os.path.basename(sample_batched['path']).replace('.jpg', '.parquet')}"
            ...
            if df.shape[0] >10:
                status = "success"
                filepath= f"/home/ray/train_data/finalcaption_{os.path.basename(sample_batched['path']).replace('.jpg', '.parquet')}"
...
#                        use_listings_cache=False,
    ...
    imgs = s3.find('akash-thumper-v1-training-images')
    # imgs = s3.ls('akash-thumper-v1-training-images', refresh=True, versions=False)

    caption_df = pd.read_parquet(#args.caption_filpath
                             "s3://akash-thumper-v1-training-data/captionfile_v1.parquet", storage_options={"key":'005d438bd2ef4c70000000002',  
                                                                    "secret":'K005fyvkZBOwFCUsl4z6af3xRgopy88',
                                                                    "endpoint_url":'https://s3.us-east-005.backblazeb2.com'})
    
    img_filenames= [os.path.basename(img) for img in imgs]
    already_captioned_img_paths = caption_df["file_name"].tolist()

    already_captioned_imgs= [os.path.basename(img) for img in already_captioned_img_paths]

    remainind_imgs = []
    remainind_img_paths= []

    for i,img in enumerate(img_filenames):
        if img not in already_captioned_imgs and "jpg" in img:
            remainind_imgs.append(img)
            remainind_img_paths.append(imgs[i])


    print("files", remainind_img_paths)

    dir_chunks =list(more_itertools.divide(args.worker, remainind_img_paths ))
    dir_chunks = [list(achunk) for achunk in  dir_chunks]
    print("dir chunk", dir_chunks, len(dir_chunks))

    ray.init(address="auto")

    actors = [Actor.remote(args=args, files=dirs) for dirs in dir_chunks]
    jobs = []
    valid_actors =[]
    for actor in actors:
...
# 17030RuntimeError: CUDA error: uncorrectable ECC error encountered