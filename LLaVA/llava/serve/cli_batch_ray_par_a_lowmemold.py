...
if __name__ == "__main__":
    ...
    captions = s3.ls('akash-thumper-v1-captions', refresh=True, versions=False)
    captions= [os.path.basename(cap) for cap in captions]
    captions= [(cap).replace("finalcaption_","").replace(".parquet", "") for cap in captions]
    ...
    for result in paginator.paginate(Bucket='akash-thumper-v1-training-images', Delimiter='/'):
        for prefix in result.get('CommonPrefixes'):
            aprefix = prefix.get('Prefix')
            aprefix = aprefix.replace("/", "")
            if (aprefix not in [ '_tmp/', 'ixArt-alpha/', 'PixArt-alpha/', 'lava-v1.5-7b/', 'llava-v1.5-7b/']) and not s3.exists(f"akash-thumper-v1-captions/finalcaption_{aprefix}.parquet") and int(aprefix)>171:
                if aprefix in captions:
                    print(f"{aprefix} found already skipping")
                else:  
                    dirlist.append(aprefix)
