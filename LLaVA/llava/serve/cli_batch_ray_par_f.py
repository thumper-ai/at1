...
def main(args, adirin:str):
    # Model
    # device_count = torch.cuda.device_count()
    # assert device_count >0 , "device count is 0"
    ...
    if not os.path.exists('/home/ray/models/llava-v1.5-7b/pytorch_model-00002-of-00002.bin'):
    ...
    # paths = glob.glob(os.path.join(f"{args.image_folder}/{adirin}","*.jpg"), recursive=True)
    ...
    tokenizer, model, image_processor, context_len = load_pretrained_model(model_path=args.model_path, model_base=args.model_base, model_name=model_name, load_8bit= args.load_8bit,load_4bit= args.load_4bit, device=device)
    ...
    paths = glob.glob(os.path.join(f"{args.image_folder}/{adirin}","*.png"), recursive=True)
    paths += glob.glob(os.path.join(f"{args.image_folder}/{adirin}","*.jpg"), recursive=True)
    paths += glob.glob(os.path.join(f"{args.image_folder}/{adirin}" ,"*.jpeg"), recursive=True)
    paths += glob.glob(os.path.join(f"{args.image_folder}/{adirin}","*.webp"), recursive=True)
    ...
    with torch.inference_mode():
        for i_batch, sample_batched in enumerate(tqdm(dataloader)):
            ...
            if i_batch % 1000 ==0:
                results={"file_name":file_names,"text":texts}
                pd.DataFrame(results).to_parquet(args.output_csv,index=False)
                upload_file(client, bucket="akash-thumper-v1-training-data" , key= os.path.basename(args.output_csv.replace(".parquet", f"temp_{i_batch}_{adirin}.parquet"), filepath= args.output_csv.replace(".parquet", f"temp_{i_batch}_{adirin}.parquet")))
    results={"file_name":file_names,"text":texts}
    pd.DataFrame(results).to_parquet(f"/tmp/final_{adirin}.parquet",index=False)
    upload_file(client, bucket="s3://akash-thumper-v1-training-data" , key= os.path.basename(f"{args.image_folder}/{adirin}/final_{i_batch}_{adirin}.parquet"), filepath= f"{args.image_folder}/{adirin}/final_{i_batch}_{adirin}.parquet")
...
if __name__ == "__main__":
    ...
    # pathlist =list(chunked(paths, args.worker))
    dirlist = list(pathlib.Path("/home/ray/train_data").iterdir())
    print("dirs", dirlist)
    ray.init(address="auto")
    jobs=[]
    for adirin in dirlist:
        jobs.append(main.remote(args, adirin ))
    ray.get(jobs)
