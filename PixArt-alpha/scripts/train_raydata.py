...
if __name__ == '__main__':
    ...
    def train_func(config):
        ...
        if config.multi_scale:
            ...
            train_dataloader = build_dataloader(dataset, batch_sampler=batch_sampler, num_workers=0)
        else:
            train_dataloader = build_dataloader(dataset, num_workers=0, batch_size=8, #config.train_batch_size,
                                                 shuffle=True)
    ...
    ds =ray.data.read_numpy(f"s3://akash-thumper-v1-combined-datas?endpoint_override={os.environ['S3_ENDPOINT_URL']}", partition_filter=".npz") 
