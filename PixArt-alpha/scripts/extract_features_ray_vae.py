...
def extract_caption_t5(fp, save_path_root, model_path_dir):
    ...
    with torch.no_grad():
        for a in tqdm(range(train_data.shape[0])):
            ...
            save_path = os.path.join(vae_save_dir, os.path.basename(train_data['file_name'][a]).replace('.png', '.npz').replace('.jpg', '.npz').replace('.webp', '.npz').replace('.jpeg', '.npz'))
...
class Actor:
    def __init__(self, args, dirs, df):
        ...
        self.vae = AutoencoderKL.from_pretrained(self.model_path_dir).to(self.device)
    ...
    def inference(self):
        ...
        with torch.no_grad():
            for a in tqdm(range(self.df.shape[0])):
                ...
                # filename = os.path.basename(filepath).replace('.png', '.npy').replace('.jpg', '.npy').replace('.webp', '.npy').replace('.jpeg', '.npy')
                # shardpath = filepath.replace(os.path.basename(filepath), "").replace("/home/ray/train_data/","")


                # filepath = os.path.basename(filepath).replace('.png', '.npy').replace('.jpg', '.npy').replace('.webp', '.npy').replace('.jpeg', '.npy').replace("/home/ray/train_data/", self.vae_save_dir)
                # save_path = f"{self.vae_save_dir}/{shardpath}/{filename}"
                image_name = os.path.basename(str(train_data['file_name'].values[a]))
                save_path = os.path.join(self.vae_save_dir, image_name.replace('.jpg', '.npy'))
                # save_folder =  f"{self.vae_save_dir}/{shardpath}/"
                # if not os.path.exists(save_folder):
                #     os.path.mkdirs(save_folder, exist_ok=True)
                # filepath.replace("/home/ray/train_data/", self.vae_save_dir)

                # caption = str(train_data['text'].values[a]).strip()
                # caption = [caption]

                # save_path = os.path.join(self.vae_save_dir, os.path.basename(train_data['file_name'][a]).replace('.png', '.npz').replace('.jpg', '.npz').replace('.webp', '.npz').replace('.jpeg', '.npz'))
                ...
                # except Exception as e:
                #     print(e)
                image_name = train_data['file_name'].values[a]
                if os.path.exists(self.save_path):
                            continue
                try:
                    img = Image.open(str(train_data['file_name'].values[a]))
                    img = self.transform(img).to(device)[None]

                    with torch.no_grad():
                        posterior = self.vae.encode(img).latent_dist
                        z = torch.cat([posterior.mean, posterior.std], dim=1).detach().cpu().numpy()

                    np.save(self.save_path, z)
                    print(save_path)
                except Exception as e:
                    print(e)
                    print(image_name)
