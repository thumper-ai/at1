...
def train_func(config):
    ...
    if not os.path.exists('/home/ray/models'):
        os.makedirs('/home/ray/models', exist_ok=True)
    ...
    if not os.path.exists('/home/ray/train_data/local2'):
        os.makedirs('/home/ray/train_data/local2', exist_ok=True)

    for a in range(0, 2210):
        if not os.path.exists(f"/home/ray/train_data//local2/wds_at1--{a}.tar"):
            s3.get(f"s3://akash-thumper-v3-wds-256/wds_at1--{a}.tar", "/home/ray/train_data/local2")    
    ...
    for i in range(1, 1000+1):
        ...
        for batch in train_loader:
            x = batch["image_latents.pyd"].squeeze().to(accelerator.device)
            y=  batch["text_encodings.pyd"].squeeze().to(accelerator.device)
...
# def main(config: ModelConfig):
    config = ModelConfig( run_id="zyn1tvk1", model_name="dev123",
    ...
    if not os.path.exists('/home/ray/train_data/local2/'):
        os.makedirs('/home/ray/train_data/local2/')
    ...
    train_dataset = (
        ray.data.read_webdataset( '/home/ray/train_data/local2',
