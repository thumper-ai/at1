...
def train_func(config):
    ...
    accelerator = Accelerator(mixed_precision="fp16", log_with="wandb", kwargs_handlers=[ipg_handler])
    ...
    for a in range(26):
        ...
        if not os.path.exists(image_latent_path):
            s3.get(f"akash-thumper-v2-wds-256/image_latents_{a}.npy", "/home/ray/train_data")
        if not os.path.exists(text_emb_path):
            s3.get(f"akash-thumper-v2-wds-256/text_encodings_{a}.npy", "/home/ray/train_data")
        try:
    ...
    #     # collate_fn= collate,
    #     batch_size=64, dtypes=torch.float16
    #     #dtypes={"image_latents.npy": torch.float16, "text_encodings.npy": torch.float16} #dtypes={"vae": torch.float32, "txt_features": torch.float32, "attention_mask": torch.int32}
    # )
    ...
    vae = diffusers.models.AutoencoderKL.from_pretrained("madebyollin/sdxl-vae-fp16-fix", cache_dir= "/home/ray/models",
    ...
    for i in range(1, 1000):
        ...
        for x,label in train_loader:
            ...
            # x = batch["image_latents.npy"].squeeze()#.to(accelerator.device)
            # label=  batch["text_encodings.npy"].squeeze()#.to(accelerator.device)
            ...
            x_noisy = noise_level.contiguous().view(-1,1,1,1)*noise + signal_level.contiguous().view(-1,1,1,1)*x
            ...
            if global_step % 1000 == 0:
                ...
                if accelerator.is_main_process:
                    ...
                    # with tempfile.TemporaryDirectory() as temp_checkpoint_dir:
                        ##eval and saving:
                    out = eval_gen(diffuser=diffuser, labels=label[0:64,])
                    out.save('img.jpg')
                    accelerator.log({f"step: {global_step+1}": wandb.Image("img.jpg")})
                    ...
                    # filepath =  f"{temp_checkpoint_dir}/{config.model_name}_ckpt_{i}.bin"
            ...
            with accelerator.accumulate(model):
                ...
                pred = model(x_noisy, noise_level.contiguous().view(-1,1), label)
                accelerator.print("pred", pred.dtype, x.dtype)
                # loss = loss_fn(pred, x.float())
                loss = loss_fn(pred.half(), x)

                accelerator.log({"train_loss":loss.item()}, step=global_step+1)
...
# def main(config: ModelConfig):
    config = ModelConfig( run_id="zyn1tvk1", 
    ...
    #                              "s3://akash-thumper-v3-wds-256?scheme=http&endpoint_override=ENTER_URL_HERE" ,
    #                              verbose_open= False, parallelism = 64
    ...
    #         s3.get(f"akash-thumper-v2-wds-256/text_encodings_{a}.npy", "/home/ray/train_data")
    #     try:
    #         lt_img = np.load(image_latent_path)
    ...
    # latent_train_data= torch.cat(latent_train_data)
    # train_label_embeddings= torch.cat(train_label_embeddings)
    # print("latent img size ",latent_train_data.size())
    # dataset = TensorDataset(latent_train_data, train_label_embeddings)
    # train_loader = DataLoader(dataset, batch_size=config.batch_size, shuffle=True)

    # del latent_train_data
    # del train_label_embeddings
    # train_loader = DataLoader(dataset, batch_size=config.batch_size, shuffle=True)
...
# main(ModelConfig() )
    ...
    # if not config.from_scratch:
    #     accelerator.print("Loading Model:")
    #     wandb.restore(config.model_name, run_path=f"apapiu/at1256/runs/{config.run_id}",
    #                   replace=True)
    #     full_state_dict = torch.load(config.model_name)
    #     model.load_state_dict(full_state_dict['model_ema'])
    #     optimizer.load_state_dict(full_state_dict['opt_state'])
