...
if __name__ == '__main__':
    ...
    def train_func(config):
        ...
        s3 = s3fs.S3FileSystem(#anon=True, 
            ...
            #                        secret='*****************',
                                endpoint_url='ENTER_URL_HERE', version_aware=True)
        s3.get('akash-thumper-v1-checkpoints/TorchTrainer_2023-12-15_12-01-08/TorchTrainer_b002f_00000_0_2023-12-15_12-01-08/checkpoint_000000/epoch_50_step_1250.pth',
    ...
    #   num_canonical_nodes: 64
        ...
        for epoch in range(start_epoch + 1, config.num_epochs + 1):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                #     z = batch[0]
                # else:
                # print("input", batch["vae_mean"].size(),  batch["vae_std"].size(), batch["txt_features"].size())

                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=config.mixed_precision == 'fp16'):
                        vae_mean, vae_std =  batch["vae"].chunk( 2, dim=1)
                        print("dl", batch["vae"].size(), vae_mean.size() , vae_std.size())
                ...
                print(z.size(), vae_mean.size(),   sample.size(), batch["txt_features"].size())
            
                clean_images = (z * config.scale_factor).squeeze()#.to(ray.train.device)
                y = batch["txt_features"].squeeze().reshape(-1, 1, 120, 4096)#.to(ray.train.device) #batch[1]
                y_mask = batch["attention_mask"].squeeze().reshape(-1, 1, 1, 120)#.to(ray.train.device)#batch[2]
                # data_info = {}#batch[3]
                data_info = {'img_hw': torch.tensor([256, 256], dtype=torch.float32),
                ...
                optimizer.zero_grad()
                assert y.size()[0] ==bs, f"error {y.size()[0]} {bs}"
                assert y.size()[0] ==bs, f"error {y_mask.size()[0]} {bs}"
                assert timesteps.size()[0] ==bs, f"error {timesteps.size()[0]} {bs}"
                # input torch.Size([4, 4, 32, 32]) torch.Size([4]) torch.Size([4, 1, 120, 4096]) torch.Size([4, 1, 1, 120]) v 7(=c0)
                ...
                ray.train.report({"loss": current_loss, "step": (epoch - 1) * len(train_dataloader) + 1, "epoch":epoch })
            ...
            if ray.train.get_context().get_world_rank() == 0:
                ...
                if epoch % config.save_model_epochs == 0 or epoch == config.num_epochs or epoch % 2 ==0:
                    ...
                    # os.system(f"b2 sync /home/ray/ray_results/ b2://akash-thumper-v1-checkpoints --skipNewer")
