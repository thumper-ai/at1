...
if __name__ == '__main__':
    ...
    def train_iter(batch, train_diffusion, lr_scheduler, model, optimizer, global_step, step, epoch, start_step, log_buffer, logger, model_ema,
                    accelerator, dataloader_len , time_start, total_steps, last_tic, data_time_all):
        # with torch.no_grad():
        #     with torch.cuda.amp.autocast(enabled=config.mixed_precision == 'fp16'):
        #         vae_mean, vae_std =  batch["vae_features.pyd"].chunk( 2, dim=1)
        #         # if  len(batch["vae_mean"].size())>4:
        #             # vae_mean = batch["vae_mean"].drop_axes(1)
        #         sample = randn_tensor(vae_mean.squeeze().size(), generator=None, device=batch["vae_features.pyd"].device, dtype=batch["vae_features.pyd"].dtype)
        #         z = vae_mean.squeeze() + vae_std.squeeze() * sample
        # clean_images = (z * config.scale_factor).squeeze().to(accelerator.device)
        # y = batch["txt_features.pyd"].squeeze().reshape(-1, 1, 120, 4096).to(accelerator.device)
        # y_mask = batch["attention_mask.pyd"].squeeze().reshape(-1, 1, 1, 120).to(accelerator.device)
        # # data_info = {}#batch[3]
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=config.mixed_precision == 'fp16'):
                ...
                # if  len(batch["vae_mean"].size())>4:
                    # vae_mean = batch["vae_mean"].drop_axes(1)
                sample = randn_tensor(vae_mean.size(), generator=None, device=batch["vae_features.pyd"].device, dtype=batch["vae_features.pyd"].dtype)
                z = vae_mean+ vae_std* sample
        clean_images = (z * config.scale_factor).to(accelerator.device)
        y = batch["txt_features.pyd"].squeeze().reshape(-1, 1, 120, 4096).to(accelerator.device)
        y_mask = batch["attention_mask.pyd"].reshape(-1, 1, 1, 120).to(accelerator.device)
        # data_info = {}#batch[3]
        data_info = {'img_hw': torch.tensor([256, 256], dtype=torch.float32), 'aspect_ratio': torch.tensor(1.),  'mask_type':  'null' }
        ...
        if (step + 1) % config.log_interval == 0 or (step + 1) == 1:
            ...
            log_buffer.average()
            info = f"Step/Epoch [{(epoch-1)*int(dataloader_len)+step+1}/{epoch}][{step + 1}/{int(dataloader_len)}]:total_eta: {eta}, " \
    ...
    def train_func(config):
        ...
        # torch.cuda.set_device(ray.train.get_device())

        # from numba import cuda 
        # device=cuda.select_device(int(os.environ["LOCAL_RANK"])) #ray.train.get_context().get_device())
        # # device = cuda.get_current_device(ray.train.get_device())
        # device.reset()
        # cuda.close()
        ...
        s3 = s3fs.S3FileSystem(#anon=True, 
            ...
            #                        secret='*****************',
                                endpoint_url='ENTER_URL_HERE', version_aware=True)
        # if not os.path.exists("/home/ray/ray_results/epoch_56_step_480.pth"):
            ...
            # s3.get('akash-thumper-v1-checkpoints/TorchTrainer_2023-12-27_15-34-10/TorchTrainer_70101_00000_0_2023-12-27_15-34-11/checkpoint_000022/epoch_33_step_7107.pth',
            # s3.get('akash-thumper-v1-checkpoints/TorchTrainer_2023-12-29_13-28-46/TorchTrainer_4020e_00000_0_2023-12-29_13-28-47/checkpoint_000008/epoch_42_step_2781.pth',
        # if ( str(ray.train.get_context().get_local_rank() ) =="0"  or ray.train.get_context().get_local_rank() ==0 ):
        s3.get('akash-thumper-v1-checkpoints/TorchTrainer_2024-01-09_19-50-16/TorchTrainer_5eaa7_00000_0_2024-01-09_19-50-18/checkpoint_000000/epoch_56_step_480.pth',
                "/home/ray/ray_results/")
        # s3.get('akash-thumper-v1-checkpoints/TorchTrainer_2023-12-30_14-11-49/TorchTrainer_6e104_00000_0_2023-12-30_14-11-50/checkpoint_000010/epoch_53_step_3399.pth',
        #     "/home/ray/ray_results/")
        # s3.get('akash-thumper-v1-checkpoints/TorchTrainer_2024-01-05_14-27-43/TorchTrainer_a4c3d_00000_0_2024-01-05_14-27-43/checkpoint_000001/epoch_55_step_960.pth',
        # "/home/ray/ray_results/")
        ...
        # "/home/ray/ray_results/")
        # # download_dataset()
        # logger.info(f"before ws, lws, rank, {str(os.environ['WORLD_SIZE'])} {str(os.environ['LOCAL_WORLD_SIZE'])} {str(os.environ['RANK'])}  {str(os.environ['MASTER_ADDR'])}  {str(os.environ['MASTER_PORT'])}" )
        # os.environ['WORLD_SIZE'] = str(ray.train.get_context().get_world_size())
        # os.environ['LOCAL_WORLD_SIZE'] = str(ray.train.get_context().get_local_world_size() )
        # os.environ['RANK'] = str(ray.train.get_context().get_local_rank() )
        # os.environ['RANK'] = str(ray.train.get_context().get_world_rank() )
        # get_world_rank, get_world_size, get_node_rank
        # logger.info(f"after ws, lws, rank, {str(os.environ['WORLD_SIZE'])} {str(os.environ['LOCAL_WORLD_SIZE'])} {str(os.environ['RANK'])} {str(os.environ['MASTER_ADDR'])}  {str(os.environ['MASTER_PORT'])}" )
        ...
        # def collate(batch):
        #     # batch["vae"] = torch.from_numpy(  batch("vae") )
        #     # batch["txt_features"] = torch.from_numpy(  batch("txt_features") )
        ...
        train_dataloader = train_data_shard.iter_torch_batches(
            prefetch_batches =  1, device = accelerator.device, #collate_fn= collate,
            batch_size=8, #dtypes={"vae": torch.float32, "txt_features": torch.float32, "attention_mask": torch.int32}
        )
        ...
        #                                         , local_files_only=True)
            # vae = vae.to(ray.accelerator.device)
    ...
    #   num_canonical_nodes: 64
        ...
        start_step = start_epoch * train_dataset_len
        global_step = 0
        total_steps = train_dataset_len * config.num_epochs
        current_loss =1.0
        ...
        for epoch in range(start_epoch + 1, config.num_epochs + 1):
            ...
            for step, batch in enumerate(train_dataloader):
                metrics = train_iter(batch, train_diffusion, lr_scheduler, model, optimizer, global_step, step, epoch, start_step, 
                                     log_buffer, logger, model_ema, accelerator, train_dataset_len,  time_start, total_steps, last_tic, data_time_all)
                # ray.train.report(metrics)
                # accelerator.print(f"losses {losses}")
                losses += metrics["loss"]
                accelerator.print(f"losses { float(metrics['loss'])}")

                global_step += 1
                
            metrics = {"loss": float(losses/train_dataset_len), "step": int(global_step) +int(step), "epoch": int(epoch)}
            ...
            # if ray.train.get_context().get_world_rank() == 0:
                ...
                if epoch % config.save_model_epochs == 0 or epoch == config.num_epochs or epoch % 1 ==0:
                    os.umask(0)  # file permission: 666; dir permission: 777
...
# epoch_58_step_0
    ...
    def parse_filename(row):
        # row["attention_mask.pyd"] = torch.from_numpy(pickle.loads(row["attention_mask.pyd"]))
        # row["txt_features.pyd"] = torch.from_numpy(pickle.loads(row["txt_features.pyd"]))
        # row["vae_features.pyd"] = torch.from_numpy(pickle.loads(row["vae_features.pyd"]))
        row["metadata.pyd"] = pickle.loads(row["metadata.pyd"])
        row["attention_mask.pyd"] = pickle.loads(row["attention_mask.pyd"]).squeeze(0)
        row["txt_features.pyd"] = pickle.loads(row["txt_features.pyd"]).squeeze(0)
        row["vae_features.pyd"] = pickle.loads(row["vae_features.pyd"]).squeeze(0)
                                                                            
        del row["__key__"]
        del row["__url__"]
        del row["metadata.pyd"]

        return row


    train_dataset = (
        ray.data.read_webdataset("s3://akash-thumper-v1-wds2?scheme=http&endpoint_override=ENTER_URL_HERE" ,
    ...
    # dataset_len = train_dataset.count()
    config["dataset_len"]= 400000 #int(dataset_len)
    torch.cuda.empty_cache()
    trainer = TorchTrainer(
        ...
        scaling_config=ScalingConfig(num_workers=args.n_gpu_workers, use_gpu=True, resources_per_worker={"GPU": 1, "CPU":6 }
                                     )
    )
