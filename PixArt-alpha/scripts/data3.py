...
def get_text_and_latent_embedings(dataloader, vae, model, drive_save_path):
    ...
    #                        secret='*****************',
        ...
        s3.put(fname, "akash-thumper-v3-wds-256" )
        # os.system(f"b2 upload_file akash-thumper-v1-wds3 {fname} {os.path.basename(fname)}")  # replace with your preferred command
        # os.unlink(fname)
    if not os.path.exists("/home/ray/wds256_8/"):
        os.mkdir("/home/ray/wds256_8/")
    with wds.ShardWriter(f"/home/ray/wds256_8/wds_at1-" + "-%d.tar", maxsize=50000, post=upload_shard) as writer:

        for img, label in tqdm(dataloader):
            encode_images = encode_image(img, vae).cpu().numpy()
            encode_texts = encode_text(label, model).cpu().numpy()
            ...
            # if encode_images.shape[0] ==256:
                # try:
            mds_sample = {
                            "__key__": "sample%06d" % i,
                            "image_latents.npy": encode_images,
                            'text_encodings.npy': encode_texts,
                                    }
...
if __name__ == '__main__':
    ...
    caption_col = 'text' 
    url_col = 'url'
    latent_save_path = '/home/ray/train_data/latent256_8' #could also be a path to google drive for persisting the data
    raw_imgs_save_path = '/home/ray/train_data/raw_imgs256_8' #raw imgs are not needed for training so they can be deleted once done
    use_drive = False
