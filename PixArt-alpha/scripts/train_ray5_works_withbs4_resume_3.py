...
if __name__ == '__main__':
    ...
    def download_dataset(head=False):
        ...
        captions= [captions[i] for i in range(25)]
        ...
        for i, cap in enumerate(captions):
            ...
            # if s3.exists(cap) and os.path.exists( imgfilepath_save) :
                    ...
                    if not head:
                        ...
                        print(f"downloading vae {vaefilepath} {vaefilepath_save}")
        ...
        os.chmod('/home/ray/train_data/partition/part0.txt', int('777', base=8))
    ...
    def train_func(config):
        ...
        s3 = s3fs.S3FileSystem(#anon=True, 
            ...
            #                        secret='*****************',
                                endpoint_url='ENTER_URL_HERE', version_aware=True)
        # s3.get('akash-thumper-v1-checkpoints/TorchTrainer_2023-12-08_08-07-23/TorchTrainer_e07d8_00000_0_2023-12-08_08-07-25/checkpoint_000001/epoch_4_step_536.pth',
        #        "home/ray/ray_results/TorchTrainer_2023-12-08_08-07-23/TorchTrainer_e07d8_00000_0_2023-12-08_08-07-25/checkpoint_000001/epoch_4_step_536.pth")

        # if not os.path.exists("/home/ray/ray_results/epoch_4_step_536.pth"):
        s3.get('akash-thumper-v1-checkpoints/TorchTrainer_2023-12-08_08-07-23/TorchTrainer_e07d8_00000_0_2023-12-08_08-07-25/checkpoint_000000/epoch_2_step_268.pth',
                "/home/ray/ray_results/")
            # s3.get('akash-thumper-v1-checkpoints/TorchTrainer_2023-12-08_08-07-23/TorchTrainer_e07d8_00000_0_2023-12-08_08-07-25/checkpoint_000001/epoch_4_step_536.pth',
            #         "/home/ray/ray_results/")

        # s3.get('akash-thumper-v1-checkpoints/TorchTrainer_2023-12-08_08-07-23/',"home/ray/ray_results/TorchTrainer_2023-12-08_08-07-23/", recursive=True)
        ...
        if config.multi_scale:
            ...
            train_dataloader = build_dataloader(dataset, num_workers=2, batch_size=4, #config.train_batch_size,
                                                 shuffle=True)
        ...
        for epoch in range(start_epoch + 1, config.num_epochs + 1):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                grad_norm = None
                print("input", clean_images.size(), timesteps.size(), y.size(), y_mask.size(), data_info )
            ...
            if accelerator.is_main_process:
                if epoch % config.save_model_epochs == 0 or epoch == config.num_epochs:
                    ...
                    with tempfile.TemporaryDirectory() as temp_checkpoint_dir:
                        ...
                        ray.train.report({"loss": loss.item()}, checkpoint=Checkpoint.from_directory(temp_checkpoint_dir))
                        print(f"uploading {temp_checkpoint_dir}")
                        os.system(f"b2 sync {temp_checkpoint_dir} b2://akash-thumper-v1-checkpoints --skipNewer")

            # synchronize()
    os.makedirs("home/ray/ray_results/TorchTrainer_2023-12-08_08-07-23", exist_ok=True)
    # os.system(f"b2 sync b2://akash-thumper-v1-checkpoints/ /home/ray/ray_results/")
    import s3fs
    s3 = s3fs.S3FileSystem(#anon=True, 
        #                        use_listings_cache=False,
                            key='****************',
        ...
        #                        secret='*****************',
                            endpoint_url='ENTER_URL_HERE', version_aware=True)
    # captions = s3.find('akash-thumper-v1-t5-data/TorchTrainer_2023-12-08_08-07-23')
    # s3.get('akash-thumper-v1-checkpoints/TorchTrainer_2023-12-08_08-07-23/',"home/ray/ray_results/TorchTrainer_2023-12-08_08-07-23/", recursive=True)
    # s3.get('akash-thumper-v1-checkpoints/TorchTrainer_2023-12-08_08-07-23/TorchTrainer_e07d8_00000_0_2023-12-08_08-07-25/checkpoint_000001/epoch_4_step_536.pth',
    #            "home/ray/ray_results/TorchTrainer_2023-12-08_08-07-23/TorchTrainer_e07d8_00000_0_2023-12-08_08-07-25/checkpoint_000001/epoch_4_step_536.pth")
