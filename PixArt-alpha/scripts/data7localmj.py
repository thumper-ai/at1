...
def download_and_process_data(dfind, shard,
    ...
    from diffusers import AutoencoderKL
    if not os.path.exists("/home/logan/train_data"):
        os.mkdir("/home/logan/train_data")
    ...
    dataset = dataset.decode("pil").to_tuple("jpg;png", "json").map_tuple(transform, lambda x: x["prompt"])
...
if __name__ == '__main__':
    ...
    caption_col = 'prompt' 
    url_col = 'url'
    # latent_save_path = f'/home/logan/train_data/latent256_8_{}' #could also be a path to google drive for persisting the data
    # raw_imgs_save_path = f'/home/logan/train_data/raw_imgs256_8_{}' #raw imgs are not needed for training so they can be deleted once done
    use_drive = False
    ...
    for i, df in enumerate(dflist):  
        jobs.append( download_and_process_data.remote( dfind= indexes[i], shard=i,
                                latent_save_path= f"/home/logan/train_data/latent256_8_{i}",
                                raw_imgs_save_path = f"/home/logan/train_data/raw_imgs256_8_{i}",  
                                csv_path=f"/home/logan/train_data/imgs_{i}.csv",
                                image_size=256,
                                bs=128,
                                caption_col=caption_col,
                                url_col = url_col,
    ...
    #26
    for a in range(260):
        image_latent_path = f"/home/logan/train_data/image_latents_{a}.npy"
        text_emb_path = f"/home/logan/train_data/text_encodings_{a}.npy"
        if not os.path.exists(image_latent_path):
            s3.get(f"akash-thumper-v1-mj-256/image_latents_{a}.npy", "/home/logan/train_data")
        if not os.path.exists(text_emb_path):
            s3.get(f"akash-thumper-v1-mj-256/text_encodings_{a}.npy", "/home/logan/train_data")
        try:
    ...
    #                        secret='*****************',
        ...
        os.unlink(fname)        
    if not os.path.exists("/home/logan/wds256_11/"):
        os.mkdir("/home/logan/wds256_11/")
    i=0
    with wds.ShardWriter(f"/home/logan/wds256_10/wds_at1-" + "-%d.tar",  maxcount=600000, maxsize=6000000000.0, post=upload_shard) as writer:
