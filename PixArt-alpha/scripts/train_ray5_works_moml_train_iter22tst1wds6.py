...
def train_func(config):
    ...
    train_loader = train_data_shard.iter_torch_batches(
        ...
        # collate_fn= collate,
        batch_size=256, dtypes={"image_latents.npy": torch.float16, "text_encodings.npy": torch.float16} #dtypes={"vae": torch.float32, "txt_features": torch.float32, "attention_mask": torch.int32}
    )
...
# def main(config: ModelConfig):
    ...
    train_dataset = (
        ray.data.read_webdataset(
                                 "s3://akash-thumper-v2-wds-256?scheme=http&endpoint_override=ENTER_URL_HERE" ,
                                 verbose_open= False, parallelism = 32
    ...
    trainer = TorchTrainer(
        ...
        scaling_config=ScalingConfig(num_workers=26, use_gpu=True, resources_per_worker={"GPU": 1, "CPU":6 }
                                     )
    )
