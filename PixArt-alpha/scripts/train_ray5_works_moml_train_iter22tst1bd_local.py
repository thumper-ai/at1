...
if __name__ == '__main__':
    ...
    def train_func(config):
        ...
        # # download_dataset()
        print(f"before ws, lws, rank, {str(os.environ['WORLD_SIZE'])} {str(os.environ['LOCAL_WORLD_SIZE'])} {str(os.environ['RANK'])}  {str(os.environ['MASTER_ADDR'])}  {str(os.environ['MASTER_PORT'])}" )
        ...
        # get_world_rank, get_world_size, ray.train.get_context().get_node_rank() == "0"
        print(f"after ws, lws, rank, {str(os.environ['WORLD_SIZE'])} {str(os.environ['LOCAL_WORLD_SIZE'])} {str(os.environ['RANK'])} {str(os.environ['MASTER_ADDR'])}  {str(os.environ['MASTER_PORT'])}" )
        # if  ray.train.get_context().get_node_rank() == 0:
        #     os.system("b2 sync b2://akash-thumper-v1-mds2/ /home/ray/train_data/mds2 --skipNewer")

        
        streaming.base.util.clean_stale_shared_memory()
        train_dataloader = build_streaming_laion_dataloader( remote=[None],
                                                            # (remote="s3://akash-thumper-v1-mds2",
                                                                # local =f'/tmp/cache171-{str(os.environ["RANK"])}',
                                                                # local =f'/tmp/cache172-{str(os.environ["RANK"])}',
        ...
        dataset_iterator = iter(inf_loop_dataloader(train_dataloader))
        ...
        for epoch in range(start_epoch + 1, config.num_epochs + 1):
            ...
            for step, batch in enumerate(train_dataloader):
                metrics = train_iter(batch, train_diffusion, lr_scheduler, model, optimizer, global_step, step, epoch, start_step, 
                                     log_buffer, logger, model_ema, accelerator, len(train_dataloader),  time_start, total_steps, last_tic, data_time_all)
                # ray.train.report(metrics)
                # accelerator.print(f"losses {losses}")
                # losses += metrics["loss"]
                accelerator.print(f"losses {metrics['loss']}")
