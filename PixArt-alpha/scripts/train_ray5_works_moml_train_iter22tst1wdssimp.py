...
if __name__ == '__main__':
    ...
    def train_iter(batch, train_diffusion, lr_scheduler, model, optimizer, global_step, step, epoch, start_step, log_buffer, logger, model_ema,
        ...
        y = batch["txt_features.pyd"].squeeze().reshape(-1, 1, 120, 4096).to(train.torch.get_device())
        y_mask = batch["attention_mask.pyd"].squeeze().reshape(-1, 1, 1, 120).to(train.torch.get_device())
        # data_info = {}#batch[3]
        ...
        if (step + 1) % config.log_interval == 0 or (step + 1) == 1:
            ...
            data_time_all = 0
        logs.update(lr=lr)
        print(logs, step=global_step + start_step)
    ...
    def train_func(config):
        ...
        train_dataloader = train_data_shard.iter_torch_batches(
            prefetch_batches =  1, device = ray.train.torch.get_device(), #collate_fn= collate,
            batch_size=8, #32, #dtypes={"vae": torch.float32, "txt_features": torch.float32, "attention_mask": torch.int32}
        )
        ...
        for epoch in range(start_epoch + 1, config.num_epochs + 1):
            ...
            for step, batch in enumerate(train_dataloader):
                metrics = train_iter(batch, train_diffusion, lr_scheduler, model, optimizer, global_step, step, epoch, start_step, 
                                     log_buffer, logger, model_ema, None, train_dataset_len,  time_start, total_steps, last_tic, data_time_all)
                # ray.train.report(metrics)
                # accelerator.print(f"losses {losses}")
                # losses += metrics["loss"]
                print(f"losses { float(metrics['loss'])}")
