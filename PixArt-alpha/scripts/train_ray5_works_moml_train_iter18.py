        ...
        for epoch in range(start_epoch + 1, config.num_epochs + 1):
                ...
                if load_vae_feat:
                    ...
                    with torch.no_grad():
                        with torch.cuda.amp.autocast(enabled=config.mixed_precision == 'fp16'):
                            ...
                            # mean, std = torch.from_numpy(np.load(path).squeeze()).chunk(2)
                            sample = randn_tensor(batch["vae_mean"].size(), generator=None, device=batch["vae_mean"].device, dtype=batch["vae_mean"].dtype)
                            z = batch["vae_mean"] + batch["vae_std"] * sample

                clean_images = (z * config.scale_factor).to(ray.train.device)
                y = batch["txt_feature"].to(ray.train.device) #batch[1]
                y_mask = batch["attention_mask"].to(ray.train.device)#batch[2]
                data_info = {}#batch[3]
            ...
            if ray.train.get_context().get_world_rank() == 0:
                if epoch % config.save_model_epochs == 0 or epoch == config.num_epochs or epoch % 2 ==0:
                    ...
                    ray.train.report({"loss": loss.item(), "step": (epoch - 1) * len(train_dataloader) + step + 1, "epoch":epoch },  checkpoint=Checkpoint.from_directory(save_dir))
            # if accelerator.is_main_process:
            #     if epoch % config.save_model_epochs == 0 or epoch == config.num_epochs or epoch % 2 ==0:
            #         os.umask(0)  # file permission: 666; dir permission: 777
            #         save_dir = f"/home/ray/ray_results/checkpoint_{epoch}/"
