...
def train_func(config):
    ...
    # )
    # latent_train_data = torch.tensor(np.load("/home/logan/thumperai/small_ldt/image_latents256.npy"), dtype=torch.float32)
    # train_label_embeddings = torch.tensor(np.load("/home/logan/thumperai/small_ldt/orig_text_encodings256.npy"), dtype=torch.float32)
    ...
    for a in range(1,15):
        ...
        if not os.path.exists(image_latent_path):
            s3.get(f"akash-thumper-v1-face-256/image_latents_{a}.npy", "/home/ray/train_data")
    ...
    # latent_train_data = torch.tensor(np.load("/home/logan/thumperai/small_ldt/mj_latents.npy"), dtype=torch.float32)
    # train_label_embeddings = torch.tensor(np.load("/home/logan/thumperai/small_ldt/mj_text_emb.npy"), dtype=torch.float32)
...
# def main(config: ModelConfig):
    config = ModelConfig( run_id="ixs75trg", model_name= "dev123_ckpt_996.bin_ckpt_172.bin_ckpt_761.bin_ckpt_568.bin_ckpt_2.bin_ckpt_57.bin_ckpt_79.bin_ckpt_109.bin_ckpt_23.bin_ckpt_7.bin",
    ...
    for a in range(1,15):
        image_latent_path = f"/home/ray/train_data/image_latents_{a}.npy"
        text_emb_path = f"/home/ray/train_data/text_encodings_{a}.npy"
        print(a, image_latent_path, text_emb_path)
        if not os.path.exists(image_latent_path):
            s3.get(f"akash-thumper-v1-face-256/image_latents_{a}.npy", "/home/ray/train_data")
        if not os.path.exists(text_emb_path):
            s3.get(f"akash-thumper-v1-face-256/text_encodings_{a}.npy", "/home/ray/train_data")
    ...
    latent_train_data= torch.cat(latent_train_data).squeeze()
    train_label_embeddings= torch.cat(train_label_embeddings).squeeze()
    print("final size", latent_train_data.size(), train_label_embeddings.size())
    ...
    trainer = TorchTrainer(
        ...
        scaling_config=ScalingConfig(num_workers=1, use_gpu=True, resources_per_worker={"GPU": 1, "CPU":6 }
                                     )
    )
