...
def download_and_process_data(dfind, shard,
    ...
    if not os.path.exists("/home/ray/train_data/cc_face_300k.parquet"):
        df = pd.read_parquet("s3://akash-thumper-v1-training-data/cc_face_300k.parquet",    
                                storage_options={"key":'****************',
                                                "secret":'****************',
                                                "endpoint_url":'ENTER_URL_HERE'})
        df.to_parquet("/home/ray/train_data/cc_face_300k.parquet")
    else:
        df= pd.read_parquet("/home/ray/train_data/cc_face_300k.parquet")
    df.rename(columns={"URL":"url", "TEXT":"prompt"}, inplace=True)
    ...
    if save_data:
        np.save(os.path.join(latent_save_path, f'image_latentsf_{shard}.npy'), image_latents.numpy())
        np.save(os.path.join(latent_save_path, f'text_encodingsf_{shard}.npy'), text_encodings.numpy())
        ...
        s3.put(os.path.join(latent_save_path, f'image_latentsf_{shard}.npy'), "akash-thumper-v1-face-256")
        s3.put(os.path.join(latent_save_path, f'text_encodingsf_{shard}.npy'), "akash-thumper-v1-face-256" )
...
if __name__ == '__main__':
    ...
    #     s3.get(f"akash-thumper-v1-face-256/text_encodings_{a}.npy", "/home/ray/train_data")
    
    indexlst= list(range(df.shape[0]))
    dir_chunks = list(more_itertools.divide(1 ,indexlst ))
    ...
    for i, df in enumerate(dflist):  
        jobs.append( download_and_process_data( dfind= indexes[i], shard=i,
                                latent_save_path= f"/home/ray/train_data/latent256f",
                                raw_imgs_save_path = f"/home/ray/train_data/raw_imgs256f",  
                                csv_path=f"/home/ray/train_data/imgsf_{i}.csv",
                                image_size=256,
                                bs=128,
                                caption_col=caption_col,
                                url_col = url_col,
    ...
    #26
    for a in range(52):
        image_latent_path = f"/home/ray/train_data/latent256f/image_latentsf_{a}.npy"
        text_emb_path = f"/home/ray/train_data/latent256f/text_encodingsf_{a}.npy"
        if not os.path.exists(image_latent_path):
            s3.get(f"akash-thumper-v1-face-256/image_latentsf_{a}.npy", "/home/ray/train_data/latent256f")
        if not os.path.exists(text_emb_path):
            s3.get(f"akash-thumper-v1-face-256/text_encodingsf_{a}.npy", "/home/ray/train_data/latent256f")
        try:
    ...
    #                        secret='*****************',
                    endpoint_url='ENTER_URL_HERE', version_aware=True)
        s3.put(fname, "akash-thumper-v1-face-256" )
        # os.system(f"b2 upload_file akash-thumper-v1-wds3 {fname} {os.path.basename(fname)}")  # replace with your preferred command
        os.unlink(fname)        
    if not os.path.exists("/home/ray/wds256_12f/"):
        os.mkdir("/home/ray/wds256_12f/")
    i=0
    with wds.ShardWriter(f"/home/ray/wds256_12f/wds_at1-" + "-%d.tar",  maxcount=600000, maxsize=6000000000.0, post=upload_shard) as writer:
